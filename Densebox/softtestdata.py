# this file is used to read testdata from the paramfile
import numpy as np
import math
import os
import cv2
...
# we can use the python class to implement the test data layer and it is similar to the reading train data layer

class TeData(caffe.Layer):
     
     imgbatch=10# how many minbatches to be read every time
     
     #flipping the patch with three types
     ...
     # using to get random patch data laebl which is similar to the GettingdataAndlabel function 
     ...
     def forward(self,bottom,top):
         imagename=np.random.permutation(self.files)# random select the image 
         ...
         while(num<self.__class__.imgbatch):
              ...
              boxs=list()
              count=0
              # read the car bounding box if the image doesn't exist the car and will continuing to find in the next image
              ...
              random.shuffle(boxs)
              # the following codes is used to be let the random patch and positive patch to be 1:1
