...
class denloss(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ystar=bottom[2].data[:,0,:,:]#y*
        dstar=bottom[3].data[:,:,:,:]#d*
        batch_size=bottom[0].data.shape[0]#minibatch size 
        reg_conv = bottom[1].data# regression results
        cls_conv = bottom[0].data[:,0,:,:]# label results
        self.__class__.mlabel=bottom[4].data[:,0,:,:]# M(t) means which pixels is selected
        ...
        # select the all negtive sample pixles in all negtive samples and let the postive pixels and positive pixels to be 1:1
        ...
        # calulate the classification loss with softmax 
        cls_loss = 0.0
        correct_count = 0.0    
        num=0
        for i in range(batch_size):
        ...
        # calculate the regression loss with softmax
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        #calulate the backporpation of the classification loss
        if propagate_down[0]:
        ...
        # calulate the regression loss backpropation
        if propagate_down[1]:
