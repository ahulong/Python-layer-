...
def GettingdataAndlabel(boxs,img,box1,box2):
         ...
         for box in posboxs:
             ...
             distance=int(min((x2-x1)/4,(y2-y1)/4)*0.5)
             
             
             stx=max(centerx-distance,0)
             endx=min(centerx+distance+1,60)
             sty=max(centery-distance,0)
             endy=min(centery+distance+1,60)
...
def GetRandomPatchlabel(boxs,img,box1):
         ...
         #serach whether the patch has  positive samples
         posboxs=list()
         num=0
         maxarea=0
         maxh=0
         ...
         while num<self.__class__.imgbatch:
        	 ...
        	 y2=y1+box[3]     
              ...
              for i in range(count1):
                  ...
                  curw=np.random.randint(int(0.1*w),int(0.5*w))
                  curh=np.random.randint(int(0.15*h),int(0.5*h)) 
