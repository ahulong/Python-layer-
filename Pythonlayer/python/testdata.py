...
class TeData(caffe.Layer):
     ...
     def GettingdataAndlabel(self,boxs,img,box1,box2):
         ...
         for box in boxs:
            ...
            if self.containorcross(box1,[box[0],box[1],box[0]+box[2],box[1]+box[3]])==True:
                ...
                if area1/area2>=0.8 and area1/area2<=1.25 and area1/area3>=0.8:
         ...
         for box in posboxs:
             ...
             endx=min(centerx+distance+2,60)
             sty=max(centery-distance-2,0)
             endy=min(centery+distance+2,60)
             ...
             if self.containorcross(box1,[box[0],box[1],box[0]+box[2],box[1]+box[3]])==True:
                ...
                if area1/area3>=0.75:
                   ...
                   if area1>maxarea:
                      ...
                      maxw=x2-x1
                      maxh=y2-y1
         ratew=maxw/float(box1[2]-box1[0])
         rateh=maxh/float(box1[3]-box1[1])
        
        
                     
         if ratew>=1.0/6 and ratew<=5.0*1.25/24.0 and rateh>=1.0/6 and rateh<=5.0*1.25/24.0:
             w=box1[2]-box1[0]
             h=box1[3]-box1[1]
             ratex=float(240)/w
             ratey=float(240)/h
     ...
     def setup(self,bottom,top):
         ...
         self.files=os.listdir(self.param_str+'image/')
         print 'setup is over'
     ...
     def forward(self,bottom,top):
         ...
         while(num<self.__class__.imgbatch):
              ...
              img=np.array(cv2.imread(self.param_str+'image/'+imagename[cur]),dtype=np.float32)
              img=img/255.0-0.5
              
              boxs=list()
              count=0
              with open(self.param_str+'label/'+imagename[cur][0:-4]+'.txt',"r") as f:
                   for x in f:
                       a=x.split()
                       if a[0]=='Car':
        	 ...
        	 y2=y1+box[3]     
              ...
              while i<count1:
                  ...
                  curw=np.random.randint(int(0.10*w),int(0.4*w))
